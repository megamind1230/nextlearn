...
- **File-based content management** — Decks are plain-text =.md=, =.org=,
  or =.txt= files stored in a local vault. No proprietary database for
...
+ Key advantages of NextLearn over existing solutions:
  1) Single-application integration of reading, flashcards, quizzes, timer, and streak tracking — eliminating context switching.
  2) Plain-text file format as the source of truth — not a database dump.
  3) Keyboard profiles that match the user's existing muscle memory.
  4) Deterministic deck identity via SHA256 file path hashing — progress survives file renames and OS reinstallation.
...
- **Emacs profile** — Chords with modifier keys (=C-n=, =C-p=), prefix
  ...
  timeout, reminiscent of Emacs's =C-g= cancel mechanism.
...
*** Chosen Methodology — Kanban (Lean/Agile)
:PROPERTIES:
:CUSTOM_ID: chosen-methodology-kanban
:END:

NextLearn was developed using a Kanban-style approach, documented in
=plan-todos.org= (1351 lines of feature planning). The choice was driven
by:
...
*** Use Case Diagram (make it vertical #baka) #ok
:PROPERTIES:
...
*** Class Diagram — Core Services (need this one to vertical #baka)
:PROPERTIES:
...
named resource keys defined in =ThemeHelper.cs=.
...
3. **Start Study**: User presses =Enter= on the selected deck. The
