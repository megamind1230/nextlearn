...
\label{sec:orge8236d3}
\subsection{Problem Statement}
\label{sec:org3283be4}
\begin{itemize}
\item Modern learners face a tsunami of study tools. The typical student juggles multiple applications
\begin{itemize}
\item a note-taking app for course content
\item a flashcard app for memorization
\item a quiz platform for self-assessment
\item and a calendar or timer for study sessions.
\end{itemize}
\end{itemize}
Each tool has its own data format, its own synchronization mechanism, and
critically \textbf{\textbf{its own set of distractions}}.

\begin{itemize}
\item The core problem is twofold:
\begin{itemize}
...
or \texttt{.txt} files stored in a local vault.
\begin{itemize}
\item No proprietary database for content.
\item Your study material is portable
\item version-controllable (git),
\item and editable in any text editor.
\end{itemize}
...
\textbf{\textbf{without touching the mouse}}.
...
applications. \textbf{\textbf{IT IS POSSIBLE.}}

\newpage
\subsection{Existing Solutions — Limitations vs NextLearn Advantages}
\label{sec:orgbf2bc32}
\subsubsection{anki}
\label{sec:orge198f5a}
\begin{center}
\begin{tabular}{ll}
Aspect & Anki\\
\hline
...
\begin{enumerate}
\item Single-application gear for reading, flashcards, quizzes, timer, and streak tracking — eliminating context switching.
\item Plain-text file format as the source of truth — not a database dump.
\item Keyboard profiles that match the user's existing muscle memory(vim, emacs, vscode, custom).
\item Deterministic deck identity via SHA256 file path hashing — progress survives the file renames and OS reinstallation.
...
\end{itemize}
\subsubsection{Vim, Emacs and VScode Influence}
\label{sec:orga621120}
...
timeout, also got inspiration for Emacs's \texttt{C-g} cancel mechanism.
...
\label{sec:org0ae90b0}
\subsubsection{Chosen Methodology and choices}
\label{sec:orgc186f3a}
\paragraph{Kanban planning}
\label{sec:org1198668}
...
, also documented in
\texttt{plan-todos.org} which is for plans \& todos (1351 lines of feature planning). The choice was driven
by:
...
\end{itemize}
\paragraph{Choices}
\label{sec:orgdef3168}
\begin{itemize}
\item I chose to use the WebView because it allowing for  a customization layer on rendering (Plain-text, markdown, org-mode files)
\item I chose to make the desktop version first because:
\begin{enumerate}
\item I needed that app to work mostly offline.
\item It's more likely for students to serious study session on their laptops or PCs, not their mobiles.
\end{enumerate}
\end{itemize}
...
FYI, you can just go to \url{https://github.com/megamind1230/nextlearn}
...
\label{sec:org71b8ff2}
\subsection{Project State \& Scope}
\label{sec:org8dac427}
...
\label{sec:org7455d92}
\subsubsection{versions @10jul2026}
\label{sec:orgfda4fec}
\begin{center}
\begin{tabular}{ll}
Technology & Version\\
\hline
...
\label{sec:org4e70ae5}
\subsubsection{Core Deck Management}
\label{sec:org0a5462d}
\begin{itemize}
...
\item FR-31: 67 keyboard actions (ISTG I didn't mean to make this up) across multiple contexts (Learning,
Home, ImageOverlay, McqQuiz)
\item FR-32: Esc-based overlay closing priority chain
\end{itemize}
\subsubsection{AI Features (Google Gemini)}
\label{sec:org2729112}
\begin{itemize}
...
\item FR-41: Study streak heatmap (snake layout, 6-level orange, green border for
today)
...
\label{sec:org94994ba}
\subsection{System Architecture}
\label{sec:org3fbef8f}
...
\label{sec:org877cf4e}
\subsubsection{Use Case Diagram}
\label{sec:orgf66ab1f}
\begin{center}
...
\label{sec:orgcbfdf75}
\subsection{Unit Test Framework}
\label{sec:orga8d9fc3}
...
\label{sec:org7480bcc}
\subsection{Logo \& Branding}
\label{sec:org06c1613}
\begin{center}
...
\begin{itemize}
\item Learning (The book)
\item Continuity (The arrow)
\item Mnemontics (NextLearn ==> the next button in Study UI)
\end{itemize}
...
\textbf{Figure 12: Shortcuts handbook, auto-generated depending on current keybinding profile.}
\subsubsection{Falcon Eye (TOC)}
\label{sec:org7445e5e}
\begin{center}
...
\textbf{Figure 24: The updated default light theme.}
...
\label{sec:org0c49001}
NextLearn features a live-switchable dark/light theme system with 45+
named resource keys defined in \texttt{ThemeHelper.cs}, which could be used
for later Preset Themes features.
\subsubsection{Architecture}
\label{sec:org244594b}
\begin{enumerate}
...
\item \textbf{\textbf{Start Study}}: The app loads the deck file, parses it into pages,
builds HTML with
...
\item \textbf{\textbf{Self-Assessment}}: User opens the MCQ Quiz panel, generates usually
...
\label{sec:org3fcf096}
\subsection{Linux WebView Constraint}
\label{sec:org605235f}
\begin{itemize}
...
\item \textbf{\textbf{Risk}}: Low-medium. UI bugs are caught manually by eye during development.
\end{itemize}
\subsection{Performance on Very Large Decks}
\label{sec:orge71b34c}
\begin{itemize}
...
\label{sec:orgd2232c5}
\subsection{Conclusion}
\label{sec:org182b13f}
\begin{center}
...
\item CLI mode
\item helix extension
\item neovim extension
\item emacs extension
\end{itemize}


\newpage
\section{References \& Inspiration}
\label{sec:orgb7ba43e}
\subsection{Technology References}
\label{sec:orgbf2c3d6}
\begin{itemize}
...
\label{sec:orgbf45058}
\subsection{License}
\label{sec:org503b153}
